...
export class NeatapticService {
    ...
    createNormOrigDataMatrix(data_matrix) {
        let col_datas = this.transpose(data_matrix);
        let norm_orig_cols = _.map(col_datas,
            (col_data) => {
    ...
    start(sheet) {
        ...
        sheet = sheet.slice(0, this.num_points + 1);
        // const column_headers = sheet[0].slice();
        // const row_headers = _.map(sheet,
        //   (row) => {
        //     return row[0];
        //   }).slice();
        let data_matrix = _.map(sheet.slice(1),
            (row) => {
                return row.slice(1);
            });
        let norm_orig_data_matrix = this.createNormOrigDataMatrix(data_matrix);
        let x_datas = _.map(norm_orig_data_matrix,
            (row) => {
                return row.slice(1);
            });
        let y_data = _.map(norm_orig_data_matrix,
            (row) => {
                return row[0];
            });


        let network = new neataptic.Architect.Perceptron(this.num_input_neurons, this.num_hidden_neurons, this.num_output_neurons);
        let testing_rmse, training_y_predicted, orig_testing_y_data, orig_training_y_data, testing_y_predicted, y_predicted;
        ({
            network,
            testing_rmse,
    ...
    getPredDatas(input_data, network, output_data) {
        return _.map(input_data,
            (x_row_datas) => {
                let temp = network.activate(x_row_datas)[0] *
    ...
    customCV(x_datas, y_data, network, LOCV_percent, training_options) {
        ...
        let feed_array = _.map(training_data,
            (row) => {
                return {
                    input: _.map(row.x,
                        (x) => {
        ...
        let norm_training_x_data = _.map(training_data,
            (row) => {
                return _.map(row.x,
                    (x) => {
                        return x.norm;
        ...
        let norm_testing_x_data = _.map(testing_data,
            (row) => {
                return _.map(row.x,
                    (x) => {
                        return x.norm;
        ...
        let orig_training_y_data = _.map(training_data,
            (row) => {
                return row.y.orig;
            });

        let orig_testing_y_data = _.map(testing_data,
            (row) => {
                return row.y.orig;
            });

        let testing_rmse = this.calcRMSE(testing_y_predicted, orig_testing_y_data);
    ...
    plotEmpty(HTML) {
        let layout = {
            title: 'Predicted vs Actual',
            xaxis: { title: 'Actual Y Output' },
            yaxis: { title: 'Predicted Y Output' },
            // margin: { t: 0 }
        }
        Plotly.plot(HTML, [{
            x: [],
